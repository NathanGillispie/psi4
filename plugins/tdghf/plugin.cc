 ...
 * @BEGIN LICENSE
 *
 * tdghf by Psi4 Developer, a plugin to:
 *
...
extern "C" PSI_API
SharedWavefunction tdghf(SharedWavefunction ref_wfn,
    ...
    tdghf.common_init(ref_wfn, options);
    //tdghf.form_V();
...
void GHF::build_JK(Options& options, MintsHelper mints) {
    ...
    */
    ...
    for (int i = 0; i < nirrep_; i++) {
		...
		JKwK_[i](p, q) += std::complex<double>(Jbbr_->get(i, p, q), Jbbi_->get(i, p, q));

		//Jbb
		JKwK_[i](p+nsopi_[i], q+nsopi_[i]) += std::complex<double>(Jbbr_->get(i, p, q), Jbbi_->get(i, p, q));
		...
		//Kbb
		JKwK_[i](p+nsopi_[i], q+nsopi_[i]) -= std::complex<double>(Kbbr_->get(i, p, q), Kbbi_->get(i, p, q));

		//Kab
		JKwK_[i](p, q+nsopi_[i]) -= std::complex<double>(Kabr_->get(i, p, q), Kabi_->get(i, p, q));

		//Kba
		JKwK_[i](p+nsopi_[i], q) -= std::complex<double>(Kbar_->get(i, p, q), Kbai_->get(i, p, q));
            }
        }
    }
    //auto jkmat = jk_->D();
