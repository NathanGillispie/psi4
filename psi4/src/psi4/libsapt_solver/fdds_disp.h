...
class FDDS_Dispersion {
   protected:
    ...
    SharedM<double>atrix metric_inv_;

    // Coulomb metric
    SharedMatrix<double> metric_;

    // Coulomb metric -0.5 power
    SharedMatrix<double> metric_half_inv_;

    // Auxiliary overlap matrix
    SharedMatrix<double> aux_overlap_;

    // DFHelper object
    std::shared_ptr<DFHelper> dfh_;

    // Cache map
    std::map<std::string, SharedMatrix<double>> matrix_cache_;
   ...
   public:
    ...
    FDDS_Dispersion(std::shared_ptr<BasisSet> primary, std::shared_ptr<BasisSet> auxiliary,
                    std::map<std::string, SharedMatrix<double>> matrix_cache, std::map<std::string, SharedVector> vector_cache,
                    bool is_hybrid);
     ...
     */
    std::vector<SharedMatrix<double>> project_densities(std::vector<SharedMatrix<double>> dens);
     ...
     * @return         "PQ" amplitude 
     */
    SharedMatrix<double> form_unc_amplitude(std::string monomer, double omega);
     ...
     */
    std::map<std::string, SharedMatrix<double>> form_aux_matrices(std::string monomer, double omega);

    /**
     * Returns the metric matrix
     * @return Metric
     */
    SharedMatrix<double> metric() { return metric_; }

    /**
     * Returns the metric_inv matrix
     * @return Metric
     */
    SharedMatrix<double> metric_inv() { return metric_inv_; }

    /**
     * Returns the auxiliary overlap matrix
     * @return Overlap
     */
    SharedMatrix<double> aux_overlap() { return aux_overlap_; }

    /**
     * Returns R for QR decomposition of (ar|Q)
     * @return R_A
     */
    SharedMatrix<double> R_A() { return R_A_; }

    /**
     * Returns R for QR decomposition of (bs|Q)
     * @return R_B
     */
    SharedMatrix<double> R_B() { return R_B_; }
     ...
     * @return (R^t)^-1
     */
    SharedMatrix<double> QR(std::string monomer);

    SharedMatrix<double> get_tensor_pqQ(std::string name, std::tuple<size_t, size_t, size_t> dimensions);
