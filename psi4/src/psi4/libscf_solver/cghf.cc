        ...
        for (int h = 0; h < nirrep_; h++) {
            ...
            for (int p = 0; p < nso; p++) {
                for (int q = 0; q < nso; q++) {
                    ...
                    Hb(p, q+nso)     += Hx.get(p,q)*i; // i Hx [αβ]
                    Hb(p+nso, q)     += Hx.get(p,q)*i; // i Hx [βα]
                    Hb(p, q+nso)     += Hy.get(p,q);   //  Hy [αβ]
                    Hb(p+nso, q)     -= Hy.get(p,q);   // -Hy [βα]
                    Hb(p, q)         += Hz.get(p,q)*i; //  i Hz [αα]
                    Hb(p+nso, q+nso) -= Hz.get(p,q)*i; // -i Hz [ββ]
                }
            }
        }
    }
