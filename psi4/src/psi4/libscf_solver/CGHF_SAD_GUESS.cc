...
void CGHF::form_S() {
    ...
    for (int i = 0; i < nirrep_; i++) {
        ...
        auto S_block = einsums::Tensor<double, 2>(
                        molecule_->irrep_labels().at(i), 2*S_->rowdim(i), 2*S_->coldim(i));

        S_block.zero();
        for (int j = 0; j < S_->rowdim(i); j++) {
                for (int k = 0; k < S_->coldim(i); k++) {
                        S_block(j, k) = S_->get(i, j, k);
                        S_block(j+S_->rowdim(i), k+S_->coldim(i)) = S_->get(i, j, k);
                }
        }
...
void CGHF::form_T() {
    ...
    for (int i = 0; i < nirrep_; i++) {
        auto T_block = einsums::Tensor<double, 2>(
                        molecule_->irrep_labels().at(i), 2*T_->rowdim(i), 2*T_->coldim(i));
