...
"""Sample CGHF energies for a molecule.
...
guess.  For each system many independent noise realizations are run.  The
converged energies are summarized"""
...
def sample_energy(noise_scale, rng, options):
    """Run single CGHF calculation for molecule.
    ...
    try:
        ...
        try:
            ...
            print(f"    warning: unexpected failure: {exc}", file=sys.stderr)
    finally:
        core.CGHF.guess = original_guess

    return energy, converged, wfn


def main():
    parser = argparse.ArgumentParser(
        description="Sample CGHF energies for molecule with machine-epsilon noise injected after the guess.",
    )
    parser.add_argument(
        "--n-samples", type=int, default=30,
        help="Number of independent noise realizations per bond length (default: 20).",
    )
    parser.add_argument(
        "--noise-scale", type=float, default=2.0,
        help="Scale factor for the machine-epsilon noise (default: 2.0).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", default="cghf_noise_stability.log",
        help="Psi4 output file (default: cghf_noise_stability.log).",
    )
    ...
    print("CGHF noise-stability sampling")
    ...
    header = f"No.  {'energy':>18}  {'conv':>5}  {'spin':>5}  {'mult':>5}"
    print(header)
    print("-" * len(header))

    for sample_no in range(args.n_samples):
        e, conv, wfn = sample_energy(args.noise_scale, rng, options)
        if conv is None:
            print(f"{sample_no:3g}  {"unknown failure!":>18} {conv!r:>5}    N/A    N/A")
            continue
    ...
    header = f"\n{'mean':>18}  {'std':>12}  {'min':>18}  {'max':>18}  {'conv':>5}  {'fail':>5}  {'stable':>7}"
    print(header)
    print("-" * len(header))
    ...
    print(
        f"{mean_e:18.10f}  {std_str}  {min_e:18.10f}  {max_e:18.10f}  "
